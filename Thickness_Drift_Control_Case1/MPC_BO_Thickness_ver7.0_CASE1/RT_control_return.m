function Run_output = RT_control_return(Tref, Iref, Aref, dA)
addpath('/Users/shaoketong/tbxmanager/casadi-osx-matlabR2015a-v3.5.5')
import casadi.*
...
% flags for plotting
plots.phase = 0;
plots.controlledOutput = 0;
plots.input = 0;
plots.stateEstimate = 0;
plots.disturbanceEstimate = 0;
plots.performanceVerification = 0;
plots.processVerification = 0;
...
if strcmp(c{1}.mpc.sol.stats().return_status, 'Solve_Succeeded')
    Run_output = [max(c{1}.data.Yprs(:,1:end-1)), 1];
else
    Run_output = [max(c{1}.data.Yprs(:,1:end-1)), 0];
